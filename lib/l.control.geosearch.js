/*
 * L.Control.GeoSearch -  an address and zoom to it's location
...
L.Control.GeoSearch = L.Control.extend({
    ...
    setConfig: function (options) {
        this._config = {
            ...
            'searchLabel': options.searchLabel || 'search for an address or postcode',
            'notFoundMessage' : options.notFoundMessage || 'sorry, that address could not be found.',
            'messageHideDelay': options.messageHideDelay || 3000,
            'zoomLevel': options.zoomLevel || 11
        };
...
//            this._positionMarker.setLatLng([location.Y, location.X]);
		...
		{	
            ...
            this._positionMarker = L.marker([location.Y, location.X]).addTo(this._map);
			markers.push(this._positionMarker);
		}
        else
		{
			clearMarker();
        ...
        var pnt = new L.LatLng(point[1], point[0]);
		var results = leafletPip.pointInLayer( pnt , geojson , true );
    },
